...
function project_init(screen_width::Int, screen_height::Int)
    ...
    project_scale = 0.8*√(screen_width * screen_height)  # Geometric mean of the two dimensions.
end
...
function project_project(point::RealPoint, camera::CameraPoint)::RealPoint
    ...
    # Rotation around the horizontal axis normal to the player's direction (+ve θv moves it down).
